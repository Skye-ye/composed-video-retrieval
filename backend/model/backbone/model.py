...
class CLIP(nn.Module):
    def __init__(
        ...
        if not share_projection_layer:
            self.extra_text_projection = nn.Parameter(
    ...
    def initialize_parameters(self):
        ...
        if hasattr(self, "extra_text_projection"):
            nn.init.normal_(
                self.extra_text_projection, std=self.transformer.width**-0.5
            )
    ...
    def encode_text_img(self, text, img_tokens):
        ...
        x = torch.cat(
            [x[:, : collect_ind[0]], img_tokens, x[:, collect_ind[0] : -1]], dim=1
        )
        x = x + self.positional_embedding.type(self.dtype)
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer(x)
        ...
        for i, sample in enumerate(x):
            ...
            if isinstance(img_tokens, tuple):
                ...
                for i, index in enumerate(indexes):
                    ...
                    sample = torch.cat(
                        [sample[:, :index], img, sample[:, index + 1 :]], dim=1
                    )
            else:
                img_tokens = img_tokens.view(1, 1, -1)
                ind_insert = ind_insert.nonzero()[0]
                sample = torch.cat(
                    [sample[:, :ind_insert], img_tokens, sample[:, ind_insert + 1 :]],
                    dim=1,
                )
            new_x.append(sample)
    ...
    def encode_text_img_retrieval(self, text, img_tokens, split_ind=4, repeat=True):
        ...
        if isinstance(img_tokens, tuple):
            ...
            ind_insert = ind_insert.nonzero()[0]
            x = torch.cat(
                [x[:, :ind_insert], img_tokens, x[:, ind_insert + 1 :]], dim=1
            )
